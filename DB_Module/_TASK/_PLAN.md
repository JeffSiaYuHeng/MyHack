...
Add `POST /api/ai/cohort-summary` with seeded cohort metrics, structured narrative output, and deterministic fallback behavior.
...
- `app/api/ai/diagnose/route.ts` provides the current relationship-health AI route pattern for prompt construction, JSON parsing, and fallback behavior.
